# -*- coding: utf-8 -*-
"""
Created on Wed May 22 11:13:31 2024
...
for i,x in enumerate(mean_x):
    plt.scatter(x,mean_y[i],marker='*',label=f'Time{(integration_time-1)*i/num_bins}s',color=colors1[i])
...
plt.vlines(x=1215.67,ymax=integration_time,ymin=0,color='black')

# Label the axes
plt.xlabel('Wavelength(Angstroms)')
plt.ylabel(f'Time[s] ({(integration_time-1-t_start)/num_bins:.1f} sec bins)')
plt.title('Counts vs Wavelength and Time')
plt.minorticks_on();
#plt.gca().invert_yaxis()  # Reverse y-axis if needed

#Save counts and times locally for use in next spectrogram. Can't figure out better way yet
...
# Usage for cell (again everything called lamp but really using cell data)
directory_date = '02_05_2025'
sub_directory = 'Processed'
date = '02_05_2025'
base_dir = os.path.join(directory_date)
...
for i in range(len(binned_cell2)):
    ...
    #then last element will have wrong integration time scaling, must remove if tf > 175s
...
lyman = np.array(lyman)
f = lyman[:,:,5]

werner = np.array(werner)
f_w = werner[:,:,5]
