...
class SpectrumAnalyzer:
    ...
    def preprocess_data(self):
        ...
        self.y_lamp = (self.lamp_counts['y'].values )/(self.integration_time)  - (self.avg_dark/self.int_time_dark)
        ...
        self.y_cell = (self.cell_counts['y'].values)/(self.integration_time)  - (self.avg_dark/self.int_time_dark)
    ...
    def calibrate_wavelength(self):
        ...
        px4 = np.where(np.isclose(self.xr_lamp, 785, atol=5))[0][0]
        pxl = np.where(np.isclose(self.xr_lamp, 830, atol=2))[0][0]
        ...
        self.linfit_wlmodel = Linear1D(slope=0.349, intercept=924.3)
    ...
    def plot_data(self, save_path,name):
        ...
        ax5.plot(self.lamda_cell, self.y_e_cell, 'o-', label='Cell')
    ...
    def find_EW(self):
        ...
        return -1*EW_l, EW_cell*-1, -1*EW_2L, -1* EW_2cell, I0, I_cell,b,b2
    ...
    def analyze(self, save_path,name):
        ...
        EW_lamp1, EW_cell1, EW_lamp2,EW_cell2, I0_lamp, I0_cell,b,b2 = self.find_EW()
        ...
        print(f"Transmission: {transmission:.2f}")
        print(f"Optical Depth: {optical_depth:.2f}")
        print(f"Transmission Intensity: {transmission_intensity:.2f}")
        print(f"Intensity Cell: {I0_cell:.2f}")
        print(f"Intensity Lamp: {I0_lamp:.2f}")
...
lamp_paths = glob.glob(f'{directory_date}/{sub_directory}/{date}_lamp*')
